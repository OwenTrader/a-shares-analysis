...
- 要 Key（对话式三步，**命令全由 AI 执行，不向用户展示任何终端命令**）：
  ...
  ② 点右上角【创建 API Key】，填个别名（如 dev）后点【签发 API Key】；
  ③ 点【复制】，把 sk-fuyao- 开头的 Key **直接粘贴到对话里发给我**，剩下的我来配置。」
  （AI 侧动作：--open-admin 拉起页面 → --guide 打开截图并输出 captions 步骤 →
  等用户粘贴 → --save-key → --verify）
- Key 验证成功（原文播报 --verify 返回的 say_to_user，核心内容）：
  「✓ Key 已生效！我可以帮你：① 个股中长线深度分析（A 股/美股/港股，多智能体团队，
  产出建仓/止损/目标价方案）② 快速看盘 ③ 指数/板块/场内 ETF ④ 决策复盘。你可以直接说：
  『分析贵州茅台的中长线机会』『快速看看平安银行』『分析特斯拉』『腾讯港股怎么样』，
  或输入任何 A 股 / 指数 / 场内基金的名称或代码。另：美股/港股分析不需要任何 Key。」
